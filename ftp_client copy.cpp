...
bool uploadFile(const string& filename) {
    ...
    ifstream file(filename, ios::in | (transferMode == BINARY ? ios::binary : 0));
    if (!file) {
...
bool downloadFile(const string& filename) {
    ...
    // Nhận file
    ofstream outFile(filename, ios::in | (transferMode == BINARY ? ios::binary : 0));
    if (!outFile) {
...
void mgetFiles(const vector<string>& files) {
    for (const auto& filename : files) {
        ...
        // Tạo file để ghi
        ofstream outFile(filename, ios::out | (transferMode == BINARY ? ios::binary : 0));
        if (!outFile) {
